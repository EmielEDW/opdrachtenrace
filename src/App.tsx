...
const TEAMS_KEY = 'race:teams2';
const ACTIVE_KEY = 'race:active2';
const LOG_KEY = 'race:log2';
...
function useSharedStorage(key, def) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled && r?.value) setVal(JSON.parse(r.value));
      } catch {}
    }
    load();
  ...
  const save = async (v) => {
    setVal(v);
    try {
      await window.storage.set(key, JSON.stringify(v), true);
    } catch {}
  };
  return [val, save];
}
...
export default function App() {
  const [teams, setTeams] = useSharedStorage(TEAMS_KEY, [
    { id: 1, name: 'Team Rood', color: COLORS[0], score: 0 },
    { id: 2, name: 'Team Blauw', color: COLORS[1], score: 0 },
  ]);
  const [active, setActive] = useSharedStorage(ACTIVE_KEY, []);
  const [log, setLog] = useSharedStorage(LOG_KEY, []);

  const [view, setView] = useState('scorebord');
  const [leidingUnlocked, setLeidingUnlocked] = useState(false);
  const [pwInput, setPwInput] = useState('');
  const [pwError, setPwError] = useState(false);
  const [leidingTab, setLeidingTab] = useState('toewijzen');

  const [aTeam, setATeam] = useState(null);
  ...
  const assignOpdracht = async () => {
    ...
    const entry = {
      ...
      opdracht: aOp,
      points: aPts,
      assignedAt: new Date().toLocaleTimeString('nl-BE', {
        hour: '2-digit',
        minute: '2-digit',
      }),
    };
    await setActive([...active, entry]);
    setATeam(null);
    setAPts(null);
    setAOp(null);
  };

  const completeOpdracht = async (entry) => {
    const newTeams = teams.map((t) =>
      t.id === entry.teamId ? { ...t, score: t.score + entry.points } : t
    );
    const logEntry = {
      teamId: entry.teamId,
      teamName: teams.find((t) => t.id === entry.teamId)?.name,
      teamColor: teams.find((t) => t.id === entry.teamId)?.color,
    ...
    await setActive(active.filter((a) => a.id !== entry.id));
    await setLog([logEntry, ...log]);
  };

  const cancelOpdracht = async (entry) => {
    await setActive(active.filter((a) => a.id !== entry.id));
  };
  ...
  const addTeam = async () => {
    ...
    setEditName('');
    setEditColor(COLORS[teams.length % COLORS.length]);
  };

  const removeTeam = async (id) => {
    await setTeams(teams.filter((t) => t.id !== id));
    await setActive(active.filter((a) => a.teamId !== id));
  };

  const resetAll = async () => {
    const reset = teams.map((t) => ({ ...t, score: 0 }));
    await setTeams(reset);
    await setActive([]);
    await setLog([]);
  };

  const tryUnlock = () => {
    if (pwInput === LEIDING_PASSWORD) {
      setLeidingUnlocked(true);
      setPwError(false);
    } else {
      setPwError(true);
      setPwInput('');
    ...
    >
          ...
          {['scorebord', 'leiding'].map((v) => (
            <button
              ...
              style={{
                ...
                border: 'none',
                cursor: 'pointer',
                background: view === v ? '#fff' : '#1e1e1e',
                color: view === v ? '#0d0d0d' : '#666',
              }}
            >
      ...
      {view === 'scorebord' && (
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 16px' }}>
          ...
          >
            LIVE STAND · vernieuwt automatisch
          </div>
          <div
            style={{
              display: 'flex',
            ...
            {sorted.map((team, i) => (
              <div
                ...
                style={{
                  background: '#161616',
                  border: `2px solid ${
                    i === 0 && team.score > 0 ? team.color : '#1e1e1e'
                  }`,
                  borderRadius: 12,
                  padding: '18px 22px',
                }}
              >
                ...
                >
                  ...
                  <div style={{ flex: 1, fontSize: 22, letterSpacing: 3 }}>
                    {i === 0 && team.score > 0 ? '👑 ' : ''}
                    {team.name}
                  </div>
                  <div
                    style={{ fontSize: 44, color: team.color, lineHeight: 1 }}
                  >
                    {team.score}
                  </div>
                  <div
                ...
                {active.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <div style={{ flex: 1, fontSize: 14, color: '#bbb' }}>
                        <span style={{ color: team?.color, fontWeight: 600 }}>
                          {team?.name}
                        </span>{' '}
                        — {a.opdracht}
                      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {log.slice(0, 20).map((e, i) => (
              ...
              >
                ...
                />
                <div style={{ flex: 1, fontSize: 13, color: '#aaa' }}>
                  <span style={{ color: e.teamColor, fontWeight: 600 }}>
                    {e.teamName}
                  </span>{' '}
                  — {e.opdracht}
                </div>
      ...
      {view === 'leiding' && !leidingUnlocked && (
        ...
        >
          ...
          <div style={{ fontSize: 24, letterSpacing: 4, marginBottom: 6 }}>
            LEIDING TOEGANG
          </div>
          <div
            style={{
          ...
          <input
            ...
            style={{
              ...
              padding: '14px 16px',
              borderRadius: 8,
              border: `2px solid ${pwError ? '#E74C3C' : '#333'}`,
              background: '#161616',
              color: '#fff',
              fontFamily: "'DM Sans',sans-serif",
              fontSize: 16,
          ...
          <Btn onClick={tryUnlock} color="#fff" bg="#333" full>
            ENTER
          </Btn>
        </div>
      )}
      ...
      {view === 'leiding' && leidingUnlocked && (
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>
            {[
              ['toewijzen', '📋 Opdrachten'],
              ['teams', '👥 Teams'],
            ].map(([t, label]) => (
              <button
                key={t}
                onClick={() => setLeidingTab(t)}
                style={{
                  fontFamily: "'Bebas Neue',sans-serif",
                  letterSpacing: 2,
                  fontSize: 15,
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  background: leidingTab === t ? '#fff' : '#1e1e1e',
                  color: leidingTab === t ? '#0d0d0d' : '#666',
                }}
              >
                {label}
              </button>
            ...
            <>
              {active.length > 0 && (
                <div style={{ marginBottom: 32 }}>
                  <Label>
                    🚶 Onderweg — ✅ goedkeuren als ze terugkomen, ✖ als mislukt
                  </Label>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    {active.map((a) => {
                      ...
                      return (
                        <Card
                          key={a.id}
                          style={{ border: `1px solid ${team?.color}44` }}
                        >
                          <div
                            style={{
                              display: 'flex',
                  ...
                  <div
                    style={{ height: 1, background: '#222', margin: '28px 0' }}
                  />
                </div>
              )}
                  ...
                  {teams.map((t) => (
                    <button
                      ...
                      style={{
                        ...
                        cursor: 'pointer',
                        border: `2px solid ${
                          aTeam?.id === t.id ? t.color : '#333'
                        }`,
                        background:
                          aTeam?.id === t.id ? t.color + '22' : '#161616',
                        color: aTeam?.id === t.id ? t.color : '#888',
                      }}
                    >
                      {t.name} · {t.score} pts
                    </button>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        cursor: 'pointer',
                        border: `2px solid ${aPts === pts ? col : '#333'}`,
                        background: aPts === pts ? col + '22' : '#161616',
                        color: aPts === pts ? col : '#888',
                      }}
                    >
                      {label}
                    </button>
                    ...
                    {OPDRACHTEN[aPts].map((op) => (
                      <button
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          textAlign: 'left',
                          border: `2px solid ${aOp === op ? '#fff' : '#222'}`,
                          background: aOp === op ? '#2a2a2a' : '#161616',
                          color: aOp === op ? '#fff' : '#888',
                        }}
                      >
                        {op}
                      </button>
              ...
              <Btn
                onClick={assignOpdracht}
                disabled={!aTeam || !aPts || !aOp}
                color="#000"
                bg={aTeam?.color || '#fff'}
                full
              >
          ...
          {leidingTab === 'teams' && (
            <>
              ...
              <div style={{ borderTop: '1px solid #222', paddingTop: 20 }}>
                ...
                <Btn onClick={resetAll} color="#E74C3C" bg="#E74C3C11">
                  🗑️ Reset heel het spel
                </Btn>
              </div>
            </>
          )}
